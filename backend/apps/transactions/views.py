...
class TransactionList(CustomLoginRequiredMixin, generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        self.queryset = Transaction.objects.order_by(
            '-date').filter(user_id=request.login_user.id)
...
class TransactionReport(CustomLoginRequiredMixin, generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        end_date = datetime(current_year, current_date.month, monthrange(
            current_year, current_date.month)[-1]).date()
...
class ExpenseReport(CustomLoginRequiredMixin, generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        total_expense = sum(
            map(operator.itemgetter('total_amount'), transactions))

        for transaction in transactions:
            category = Category.objects.filter(
                id=transaction['category_id']).get()
            transaction['category_name'] = category.name
            transaction['category_color'] = category.color_code
            transaction['total_amount_percent'] = transaction['total_amount'] * \
                100 / total_expense
